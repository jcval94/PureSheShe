...
def plot_frontiers_implicit_interactive_v2(
    ...
    *,
    ...
    frontier_by_pair = {p: frontier_by_pair[p] for p in pair_keys if p in frontier_by_pair}
    bases_by_pair = {p: bases_by_pair[p] for p in pair_keys if p in bases_by_pair}
...
def plot_frontiers_implicit_interactive_v3(
    ...
    *,
    ...
    sel
        ...
        incluir ``frontier_by_pair``/``bases_by_pair`` para definir fronteras;
        si no están presentes se infieren desde ``X``/``y``.
    """
    import plotly.graph_objects as go
    import plotly.express as px
    import plotly.io as pio
    ...
    def _extract_planes_from_sel(sel_value: Any, pair_keys: Sequence[Tuple[int, int]]) -> Dict[Tuple[int, int], List[Dict[str, Any]]]:
        ...
        return planes_by_pair
    def _collect_pairs_from_sel(sel_value: Any) -> List[Tuple[int, int]]:
    ...
    def _extract_frontier_from_sel(sel_value: Any) -> Tuple[Dict[Tuple[int, int], np.ndarray], Dict[Tuple[int, int], np.ndarray]]:
        if not isinstance(sel_value, dict):
            return {}, {}
        frontier = sel_value.get("frontier_by_pair") or sel_value.get("frontier") or {}
        bases = sel_value.get("bases_by_pair") or {}
        frontier_by_pair: Dict[Tuple[int, int], np.ndarray] = {}
        bases_by_pair: Dict[Tuple[int, int], np.ndarray] = {}
        for pair, arr in (frontier or {}).items():
            norm_pair = _normalize_pair(pair)
            if norm_pair is None:
                continue
            frontier_by_pair[norm_pair] = np.asarray(arr, float)
        for pair, arr in (bases or {}).items():
            norm_pair = _normalize_pair(pair)
            if norm_pair is None:
                continue
            bases_by_pair[norm_pair] = np.asarray(arr, float)
        return frontier_by_pair, bases_by_pair

    def _fallback_frontier_from_xy(
        X: np.ndarray,
        ...
        if y is None:
            raise ValueError("Para construir fronteras sin 'records' se requiere 'y'.")
        y = np.asarray(y, int).ravel()
        rng_local = np.random.RandomState(None if random_state is None else int(random_state))
        frontier_by_pair: Dict[Tuple[int, int], np.ndarray] = {}
        bases_by_pair: Dict[Tuple[int, int], np.ndarray] = {}
        for a, b in pair_keys:
            ...
            if m <= 0:
                continue
            idx_a = rng_local.choice(Xa.shape[0], size=m, replace=False)
            idx_b = rng_local.choice(Xb.shape[0], size=m, replace=False)
            bases_by_pair[(a, b)] = Xa[idx_a]
            frontier_by_pair[(a, b)] = Xb[idx_b]
        return frontier_by_pair, bases_by_pair

    def _frontier_points_and_weights_from_frontier(frontier_by_pair: Dict[Tuple[int, int], np.ndarray]):
        F_by: Dict[Tuple[int, int], np.ndarray] = {}
        W_by: Dict[Tuple[int, int], np.ndarray] = {}
        for pair, pts in frontier_by_pair.items():
            if pts is None or pts.size == 0:
                continue
            F_by[pair] = pts
            W_by[pair] = np.ones(pts.shape[0], float) / max(1, pts.shape[0])
        return F_by, W_by
    ...
    # ---------- Pares y fronteras (sin records) ----------
    pair_candidates = _collect_pairs_from_sel(sel)
    if not pair_candidates and y is not None:
        classes = sorted(np.unique(y))
        for i, a in enumerate(classes):
            for b in classes[i + 1:]:
                pair_candidates.append((int(a), int(b)))
    pair_keys = sorted(set(pair_candidates))

    frontier_by_pair, bases_by_pair = _extract_frontier_from_sel(sel)
    if not frontier_by_pair:
        frontier_by_pair, bases_by_pair = _fallback_frontier_from_xy(X, y, pair_keys, random_state)
    if not frontier_by_pair:
        raise ValueError("No hay puntos de frontera para graficar (se requiere 'y' o frontera en sel).")
    ...
    if plane_mode == "fit_dims":
        F_for_fit, W_for_fit = _frontier_points_and_weights_from_frontier(frontier_by_pair)
    else:
        F_for_fit, W_for_fit = {}, {}
                ...
                if planes_for_p:
                    for idx_pl, meta in enumerate(planes_for_p, start=1):
                        ...
                        if is_3d:
                            ...
                            tr = go.Surface(
                                x=Xp_s, y=Yp_s, z=Zp_s, name=f"Plano {p} #{idx_pl}",
                                legendgroup=f"pair-{p}", showscale=False, opacity=op, visible=visible_value,
                                colorscale=[[0, col],[1, col]]
                            )
                        else:
                            xs = np.linspace(lo[0], hi[0], 300)
