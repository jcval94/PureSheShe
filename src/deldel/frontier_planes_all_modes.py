...
def plot_planes_with_point_lines(
    ...
    *,
    ...
    if rec_idx.size == 0:
        for i, r in enumerate(records):
            ...
            if {y0, y1} != {a_cls, b_cls}:
                continue
            raw_a = np.asarray(getattr(r, "x0"), float)
            raw_b = np.asarray(getattr(r, "x1"), float)
    ...
    if raw_A_list:
        raw_A = np.vstack(raw_A_list)
        raw_B = np.vstack(raw_B_list)
        points_for_bounds.extend([raw_A[:, dims], raw_B[:, dims]])
    if mu_sel_valid:
    ...
    finite_mask = np.isfinite(all_bounds).all(axis=1)
    if not finite_mask.any():
        if cloud_sel is None:
            raise ValueError("No hay puntos finitos para calcular límites del gráfico.")
        all_bounds = cloud_sel
    else:
        all_bounds = all_bounds[finite_mask]
