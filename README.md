...
def _metrics_pair_all(predA: np.ndarray, y_ab: np.ndarray, a: int, b: int) -> Dict[str, float]:
    ...
    bal = _balanced_accuracy_pair(predA, yab, a, b)
    ...
    *,
    ...
    if diversity_additions and (len(selected) < max_k):
        ...
        for p in pool:
            ...
            q = dict(p); q["gain"] = float(0.0); q["metrics_cumulative"] = metrics
            selected.append(q)
            used_ids.add(p["oriented_plane_id"]); added += 1
...
def prune_and_orient_planes_unified_globalmaj(
    ...
    *,
    ...
    # Prepara un diccionario de candidatos por par (permitiendo todos, pero guardando el origin para trazabilidad).
    for pair in pair_keys:
        ...
        for r in selected_global:
            rr = dict(r)
            n = rr["n_norm"]; b0 = rr["b_norm"]; side = int(rr["side"])
            rr["inequality"] = dict(
                general=_ineq_text_general(n, b0, side, feature_names),
                pretty2D=_ineq_text_2d(n, b0, side, dims=dims_for_text, feat_names=feature_names)
            )
    ...
    for k, payload in by_pair_augmented.items():
        for p in payload.get("winning_planes", []):
            winning_planes_unified.append(dict(p))
...
# --------------------------
def _side_symbol(side: int) -> str:
    return "≤" if int(side) >= 0 else "≥"

def _regions_by_oriented(sel_dict):
    """Dict: oriented_plane_id -> [region_id, ...]"""
    by_oriented = {}
    for R in sel_dict["regions_global"]["per_plane"]:
        opid = R.get("oriented_plane_id")
        if opid is None:
            continue
        by_oriented.setdefault(opid, []).append(R["region_id"])
    return by_oriented

regions_map = _regions_by_oriented(sel)

# ============================================================
# 1) Planos ganadores (unificados) con su par de origen y regiones globales asociadas
# ============================================================
print("=== Winning planes (unificados) ===")
for p in sel.get("winning_planes", []):
    plane_id = p.get("plane_id")
    opid = p.get("oriented_plane_id")
    origin_pair = tuple(p.get("origin_pair", (None, None)))
    side = _side_symbol(p.get("side", +1))
    fam = p.get("family_id")
    best_global_for = p.get("best_class")   # clase para la que ese medio-espacio es mejor globalmente (OVR)
    reg_ids = regions_map.get(opid, [])
    txt = p.get("inequality", {}).get("general")
    print(f"{plane_id} | {opid} | pair={origin_pair} | side={side} | family={fam} | best_global_for={best_global_for} | regions={reg_ids}")
    if txt:
        print("   ", txt)

# ============================================================
# 2) Todas las regiones globales (puede haber varias para la MISMA clase)
# ============================================================
print("\n=== Regiones globales (per_plane) ===")
for R in sel["regions_global"]["per_plane"]:
    rid = R["region_id"]
    pid = R.get("plane_id")
    opid = R.get("oriented_plane_id")
    c = R["class_id"]
    side = _side_symbol(R.get("side", +1))
    pretty = R.get("inequality", {}).get("pretty2D")
    print(f"{rid}: plane={pid} | oriented={opid} | clase={c} | side={side}")
    if pretty:
        print("   2D:", pretty)

# ============================================================
# 3) Lookup rápido por clase → ids de regiones
# ============================================================
target_class = 1  # cambia aquí la clase objetivo
ids_para_clase = sel["regions_global"]["per_class"].get(target_class, [])
print(f"\n=== Regiones que favorecen a la clase {target_class} ===")
print(ids_para_clase)

# ============================================================
# (Opcional) 4) Ver por par (a,b): métrica global y la regla AND resultante
# ============================================================
print("\n=== Resumen por par (a,b) ===")
for pair, payload in sel.get("by_pair_augmented", {}).items():
    overall = payload.get("metrics_overall", {})
    rule = payload.get("region_rule", {})
    ineqs = rule.get("inequalities", [])
    print(f"\nPar {pair} | balacc={overall.get('balacc', 0):.3f} | F1={overall.get('f1', 0):.3f} | recall={overall.get('recall', 0):.3f}")
    print("Regla (AND):")
    for inq in ineqs:
        sym = inq.get("side", "?")
        txt = inq.get("text", "")
        opid = inq.get("oriented_plane_id")
        print(f"  - [{opid}] {txt}  ({sym} 0)")

# ============================================================
# (Opcional) 5) Top candidatos globales (auditoría rápida)
# ============================================================
print("\n=== Top 10 candidatos globales por score ===")
cands = sorted(sel.get("candidates_global", []),
               key=lambda r: r.get("score_global", 0.0),
               reverse=True)[:10]
for c in cands:
    print(f"{c['oriented_plane_id']} | best_class={c['best_class']} | score={c['score_global']:.4f} "
          f"| size={c['region_size']} | frac={c['region_frac']:.3f} | family={c.get('family_id')} | origin={c.get('origin_pair')}")
```
